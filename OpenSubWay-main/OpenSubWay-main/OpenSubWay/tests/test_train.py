...
def test_reverser_moves_backward():
    t = train_mod.Train(build_line())
    # Move clear of the Hongqiao terminal buffer before testing reverse motion.
    t.distance += 20.0
    t.doors_open = False
...
def test_arriving_at_opposite_terminal_counts_completed_run():
    line = build_line()
    t = train_mod.Train(line)
    t.has_departed = True
    t.from_idx = line.n - 2
    t.distance = line.stations[-1].distance
    t.speed = 0.0
    t._update_progress(line.stations[-2].distance)
    assert t.completed_runs == 1
    assert t.loops_completed == 1
