...
class SwapchainBundle:
    ...
    def _choose_format(self):
        ...
        for f in formats:
            if (
                ...
                chosen = f
                break
        # The Python vulkan binding owns ``formats`` as a temporary CFFI
        # array. Returning one of its elements leaves a dangling cdata view as
        # soon as this function exits. Mesa often tolerated the stale values,
        # while NVIDIA observed imageFormat=VK_FORMAT_UNDEFINED and crashed in
        # vkCreateSwapchainKHR with 0xC0000094. Copy the scalar fields into a
        # fresh structure with independent lifetime.
        return VkSurfaceFormatKHR(
            format=int(chosen.format), colorSpace=int(chosen.colorSpace)
        )
    ...
    def create(self):
        ctx = self.ctx
        trace = os.environ.get("OPENSUBWAY_STARTUP_TRACE")
        def mark(label):
            if trace:
                print(f"[startup] swapchain: {label}", flush=True)
        mark("query surface capabilities")
        caps = self._vkGetSurfaceCaps(ctx.physical_device, ctx.surface)
        mark("choose format/present mode/extent")
        ...
        mark(f"create {extent.width}x{extent.height}, images={image_count}")
        ...
        mark(f"create {len(self.images)} image views")
        self.image_views = [
        ...
        if self.samples > VK_SAMPLE_COUNT_1_BIT:
            mark("create MSAA color image")
            self._create_color_resources()
        mark("create depth image")
