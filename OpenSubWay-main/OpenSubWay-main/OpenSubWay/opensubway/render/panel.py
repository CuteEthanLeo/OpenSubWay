...
class ControlPanel:
    ...
    def _cells(self, t):
        ...
        return [
            ("rev", f"换向 {t.reverser_label()}", rev != NEUTRAL, True,
             (0.25, 0.55, 0.30) if rev == FORWARD else
             (0.55, 0.30, 0.25) if rev == REVERSE else None),
            ("throttle_down", "牵引 -", False, True, None),
            ("throttle_show", f"牵引 {t.throttle_notch}/{n_thr}", t.throttle_notch > 0, False,
             (0.30, 0.45, 0.20) if t.throttle_notch else None),
            ("throttle_up", "牵引 +", False, True, None),
            ("brake_down", "制动 -", False, True, None),
            ("brake_show", f"制动 {t.brake_notch}/{n_brk}", t.brake_notch > 0, False,
             (0.50, 0.35, 0.15) if t.brake_notch else None),
            ("brake_up", "制动 +", False, True, None),
            ("doors", "车门 " + ("开" if t.doors_open else "关"), t.doors_open, True,
             (0.45, 0.40, 0.15) if t.doors_open else None),
            ("horn", "鸣笛", False, True, None),
        ]
