...
A 3D **Shanghai Suburban Railway Airport Link Line** driving simulator written in **Python 3.12** and
...
- **Full-length Airport Link Line**: the 58.6 km operational section from
  Hongqiao Airport Terminal 2 to Pudong Airport Terminal 1&2, with all seven
  stations at real cumulative-distance proportions. Hongqiao and Zhongchun
  Road are above ground; the remaining five stations and route are underground.
- Two running tracks, live three-aspect block signals, yard turnouts and three
  westbound AI services spaced to the public 14-minute dispatch interval.
- Four-car Class C express trains modelled at the published 24.5 m × 3.3 m ×
  3.8 m car dimensions, with 160 km/h line speed and approach limit profile.
- **Volumetric golden-hour light**: height-fog in-scattering and animated
  crepuscular sun shafts, with rounded clustered foliage and smooth normals.
...
Shaders compile to SPIR-V automatically when missing or stale (needs `glslc`
from the Vulkan SDK). Precompiled binaries are included for normal players.
To silence audio: `OPENSUBWAY_NOAUDIO=1`.
...
train) and **orbit** (free-fly around the full corridor):
